...
class AdmissionController extends Controller
{
    ...
    public function pay(Admission $admission, Request $request)
    {
        ...
        if ($finalPrice <= 0) {
            // Free or 100% discount flow
            Payment::create([
                'user_id'    => auth()->id(),
                'amount'     => 0,
                'payment_id' => 'FREE_' . strtoupper(uniqid()),
                'status'     => 'success',
            ...
            return redirect()->route('admissions.index')->with('success', 'Enrolled successfully!');
        }

        // PhonePe v2 Integration
        ...
        if ($response['success']) {
            // Find shared fee record for callback context
            $fee = Fee::where('user_id', auth()->id())
                ->where(function($q) use ($admission) {
                    if ($admission->batch_id) $q->where('batch_id', $admission->batch_id);
                    else $q->where('course_id', $admission->course_id);
        ...
        return redirect()->back()->with('error', $response['message'] ?? 'Could not initiate payment. Please try again.');
    }
