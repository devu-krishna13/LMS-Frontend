...
@section('content')
<div class="space-y-10" x-data="{ activeSection: 'my_batches', videoModalOpen: false, currentVideoId: '' }">
    <!-- Cinematic Header -->
    ...
    <div x-show="activeSection === 'my_batches'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" class="space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
            @forelse($enrolledBatches as $batch)
                @php
                    $nextSession = $batch->liveClasses->first(fn($c) => $c->isLive() || $c->start_time->isFuture());
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($pastClasses->take(6) as $class)
                @php
                    ...
                    $videoId = '';
                    if ($class->recording_url && preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $class->recording_url, $match)) {
                        $videoId = $match[1];
                    }
                @endphp
                <div class="flex items-center gap-4 p-4 bg-white rounded-[20px] border border-slate-100 hover:border-primary/20 transition-all shadow-sm group">
                    <div class="w-16 h-16 rounded-[14px] overflow-hidden shrink-0 relative">
                        ...
                        @if($isEnrolled && $class->recording_url)
                            @if($videoId)
                            <div @click="currentVideoId = '{{ $videoId }}'; videoModalOpen = true; markAttendance({{ $class->id }}, 'recording')" class="absolute inset-0 bg-navy/60 flex items-center justify-center text-white cursor-pointer hover:bg-navy/80 transition-all">
                                <i class="bi bi-play-fill text-xl"></i>
                            </div>
                            @else
                            <a href="{{ $class->recording_url }}" target="_blank" onclick="markAttendance({{ $class->id }}, 'recording')" class="absolute inset-0 bg-navy/60 flex items-center justify-center text-white hover:bg-navy/80 transition-all">
                                <i class="bi bi-play-fill text-xl"></i>
                            </a>
                            @endif
                        @else
                    ...
                    @if($isEnrolled && $class->recording_url)
                        @if($videoId)
                        <button @click="currentVideoId = '{{ $videoId }}'; videoModalOpen = true; markAttendance({{ $class->id }}, 'recording')" class="w-9 h-9 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all shadow-sm border border-emerald-100 shrink-0">
                            <i class="bi bi-play-circle-fill"></i>
                        </button>
                        @else
                        <a href="{{ $class->recording_url }}" target="_blank" onclick="markAttendance({{ $class->id }}, 'recording')" class="w-9 h-9 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all shadow-sm border border-emerald-100 shrink-0">
                            <i class="bi bi-play-circle-fill"></i>
                        </a>
                        @endif
                    @endif
    ...
    <!-- Video Player Modal -->
    <div x-show="videoModalOpen" style="display: none" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-navy/95 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-4xl bg-black rounded-[24px] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10" @click.away="videoModalOpen = false; currentVideoId = ''"
             x-transition:enter="transition ease-out duration-300"
             x-transition:enter-start="opacity-0 scale-95"
             x-transition:enter-end="opacity-100 scale-100">
            
            <!-- Close Button -->
            <button @click="videoModalOpen = false; currentVideoId = ''" class="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-black/50 text-white flex items-center justify-center hover:bg-rose-500 transition-colors">
                <i class="bi bi-x-lg"></i>
            </button>
            
            <!-- Iframe Wrapper -->
            <div class="aspect-video w-full bg-black relative z-0">
                <template x-if="currentVideoId">
                    <iframe :src="`https://www.youtube.com/embed/${currentVideoId}?autoplay=1&controls=1&modestbranding=1&rel=0`" 
...
<script>
    function markAttendance(classId, type = 'live') {
