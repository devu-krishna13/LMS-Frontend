    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
        ...
        <div class="lg:col-span-2 space-y-10">
            ...
            <div x-show="activeTab === 'assessment'" x-transition x-cloak class="space-y-6">
                ...
                <div class="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[24px] p-12 text-center">
                    <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 text-slate-200 shadow-sm">
                        <i class="bi bi-mortarboard text-2xl"></i>
                    </div>
                    <p class="text-slate-400 font-[800] text-xs uppercase tracking-widest leading-relaxed">
                        The assessment control module is now available <br> in the quick actions sidebar on the right.
                    </p>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'feedbacks'" x-transition x-cloak class="space-y-6">
                ...
                <div class="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[24px] p-12 text-center">
                    ...
                    <p class="text-slate-400 font-[800] text-xs uppercase tracking-widest leading-relaxed">
                        The student feedback reports are now available <br> in the quick actions sidebar on the right.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-8 sticky top-24">
            ...
            <!-- Final Examination Bank Widget -->
            <div x-show="activeTab === 'assessment'" x-transition x-cloak class="bg-white p-6 md:p-8 rounded-[16px] border border-slate-200 shadow-xl shadow-slate-200/20 space-y-6 relative overflow-hidden group">
                <div class="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl group-hover:bg-primary/10 transition-colors duration-700"></div>
                
                <div class="relative z-10">
                    <div class="flex items-center gap-3 pb-6 border-b border-slate-50 mb-6">
                        <div class="w-10 h-10 bg-navy/5 rounded-[10px] flex items-center justify-center text-primary">
                            <i class="bi bi-patch-question-fill text-lg"></i>
                        </div>
                        <h3 class="text-[11px] font-[900] text-navy uppercase tracking-[0.2em] mt-1">Quiz Hub</h3>
                    </div>

                    <p class="text-slate-500 font-[500] text-[13px] leading-relaxed mb-8">
                        Managing <span class="font-[900] text-navy">{{ $course->questions->count() }} high-fidelity questions</span> for this certification assessment.
                    </p>
                    
                    <div class="space-y-4">
                        <a href="{{ route('trainer.courses.quiz.index', $course->id) }}" class="w-full py-4 bg-navy text-white rounded-[12px] font-[900] text-[12px] uppercase tracking-widest hover:bg-primary transition-all shadow-xl shadow-navy/10 flex items-center justify-center gap-2">
                            Dashboard <i class="bi bi-arrow-right"></i>
                        </a>
                        <div class="text-[9px] font-[800] text-slate-300 uppercase tracking-[0.2em] text-center">
                            <i class="bi bi-shield-check text-emerald-500/50 mr-1"></i> Protocol: Verified
                        </div>
                    </div>
                </div>
            </div>

            <!-- Feedback Summary Widget (Moved here as requested) -->
            <div x-show="activeTab === 'feedbacks'" x-transition x-cloak class="bg-white p-6 md:p-8 rounded-[16px] border border-slate-200 shadow-xl shadow-slate-200/20 space-y-6 relative overflow-hidden group">
                <div class="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl group-hover:bg-emerald-500/10 transition-colors duration-700"></div>
                ...
                <div class="relative z-10">
                    <div class="flex items-center gap-3 pb-6 border-b border-slate-50 mb-6">
                        ...
                        <h3 class="text-[11px] font-[900] text-navy uppercase tracking-[0.2em] mt-1">Feedback Wall</h3>
                    </div>

                    <p class="text-slate-500 font-[500] text-[13px] leading-relaxed mb-8">
                        Over <span class="font-[900] text-navy">{{ $course->feedbacks->count() }} students</span> have shared insights on this curriculum.
                    </p>
                    
                    <div class="space-y-4">
                        <a href="{{ route('trainer.courses.feedback', $course->id) }}" class="w-full py-4 bg-navy text-white rounded-[12px] font-[900] text-[12px] uppercase tracking-widest hover:bg-primary transition-all shadow-xl shadow-navy/10 flex items-center justify-center gap-2">
                            Report Wall <i class="bi bi-arrow-right"></i>
                        </a>
                        <div class="text-[9px] font-[800] text-slate-300 uppercase tracking-[0.2em] text-center">
                            <i class="bi bi-star-fill text-primary mr-2"></i> Avg Rating: {{ number_format($course->feedbacks->avg('rating') ?: 0, 1) }}/5.0
                        </div>
                    </div>
                </div>
            </div>
