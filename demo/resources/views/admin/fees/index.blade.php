...
<div class="space-y-8">
    ...
    <!-- High-Density Financial Intelligence Grid -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- Revenue Card -->
        <div class="relative group bg-white p-6 rounded-[22px] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-[14px] flex items-center justify-center text-xl shadow-inner group-hover:bg-emerald-600 group-hover:text-white transition-all duration-500">
                        <i class="bi bi-cash-stack"></i>
                    </div>
                    <div class="text-[10px] font-[900] text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full uppercase tracking-widest">Settled</div>
                </div>
                <div class="text-[10px] font-[900] text-slate-400 uppercase tracking-widest mb-1 leading-none">Total Revenue</div>
                <div class="text-2xl font-[900] text-navy leading-none tracking-tight">₹{{ number_format($stats['total_collected']) }}</div>
            </div>
        </div>
        ...
        <div class="relative group bg-white p-6 rounded-[22px] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 bg-amber-50 text-amber-600 rounded-[14px] flex items-center justify-center text-xl shadow-inner group-hover:bg-amber-600 group-hover:text-white transition-all duration-500">
                        <i class="bi bi-clock-history"></i>
                    </div>
                    <div class="text-[10px] font-[900] text-amber-600 bg-amber-50 px-2 py-1 rounded-full uppercase tracking-widest">In Pipeline</div>
                </div>
                <div class="text-[10px] font-[900] text-slate-400 uppercase tracking-widest mb-1 leading-none">Receivables</div>
        ...
        <!-- Discounts Card -->
        <div class="relative group bg-white p-6 rounded-[22px] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden">
            <div class="absolute -top-10 -right-10 w-32 h-32 bg-blue-50 rounded-full blur-3xl opacity-50 group-hover:scale-150 transition-transform duration-700"></div>
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 bg-blue-50 text-blue-600 rounded-[14px] flex items-center justify-center text-xl shadow-inner group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                        <i class="bi bi-tags"></i>
                    </div>
                    <div class="text-[10px] font-[900] text-blue-600 bg-blue-50 px-2 py-1 rounded-full uppercase tracking-widest">Marketing</div>
                </div>
                <div class="text-[10px] font-[900] text-slate-400 uppercase tracking-widest mb-1 leading-none">Total Discounts</div>
                <div class="text-2xl font-[900] text-navy leading-none tracking-tight">₹{{ number_format($stats['total_discounts']) }}</div>
            </div>
        </div>

        <!-- Overdue Card -->
        <div class="relative group bg-navy p-6 rounded-[22px] border border-navy shadow-lg shadow-navy/10 hover:shadow-2xl transition-all duration-500 overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 bg-white/10 text-primary rounded-[14px] flex items-center justify-center text-xl shadow-inner group-hover:bg-primary group-hover:text-navy transition-all duration-500">
                        <i class="bi bi-exclamation-triangle"></i>
                    </div>
                    <div class="text-[10px] font-[900] text-primary bg-primary/10 px-2 py-1 rounded-full uppercase tracking-widest">High Risk</div>
                </div>
                <div class="text-[10px] font-[900] text-slate-400 uppercase tracking-widest mb-1 leading-none">Overdue Accounts</div>
                <div class="text-2xl font-[900] text-white leading-none tracking-tight">{{ $stats['overdue_count'] }} <span class="text-xs text-slate-500 font-bold ml-1 tracking-widest">Students</span></div>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-[11px] font-[800] text-navy uppercase tracking-wider hidden md:table-cell whitespace-nowrap">Audit Trace</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($fees as $fee)
                        @php $latestPayment = $fee->payments->first(); @endphp
                        <tr class="hover:bg-slate-50/30 transition-colors group">
                            ...
                            <td class="px-6 py-5 hidden md:table-cell">
                                @if($fee->status === 'paid' && $latestPayment)
                                    <div class="text-[11px] font-[800] text-navy uppercase mb-1 flex items-center gap-1.5">
                                        <i class="bi bi-phone-vibrate text-primary"></i> 
                                        {{ $latestPayment->payment_id ?? 'Manual Settlement' }}
                                    </div>
                                    <div class="text-[9px] text-slate-400 font-[800] uppercase tracking-widest flex items-center gap-2">
                                        @if(isset($latestPayment->response_data['data']['paymentInstrument']['type']))
                                            <span class="px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200">{{ $latestPayment->response_data['data']['paymentInstrument']['type'] }}</span>
                                        @endif
                                        Ref: {{ $latestPayment->merchant_transaction_id }}
                                    </div>
                                @else
                                    <div class="text-[14px] font-[800] text-slate-300">₹{{ number_format($fee->paid_amount) }}</div>
                                ...
                                @else
                                    <div class="flex items-center justify-end gap-2 text-slate-300">
                                        ...
                                        <div class="flex flex-col items-end">
                                            <span class="text-[10px] font-[800] uppercase tracking-widest leading-none">Settled</span>
                                            <span class="text-[8px] font-[700] uppercase text-slate-400 mt-1">{{ $latestPayment?->updated_at->format('d M, H:i') }}</span>
                                        </div>
                                    </div>
                                @endif
                            </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-8 py-20 text-center text-slate-400 italic font-bold">No fee records found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
